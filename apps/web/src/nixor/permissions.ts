...
interface PanapticonPermissionsResponse {
    ok: boolean;
...
export async function refreshNixorPermissions(force = false): Promise<NixorPermissions> {
    ...
    if (!nixorConfig?.panapticon_enabled || !matrixUserId) {
    ...
    inFlightFetch = fetch(`${baseUrl}/api/users/${encodeURIComponent(matrixUserId)}/permissions`, {
        ...
        .then(async (response) => {
            const body = (await response.json()) as PanapticonPermissionsResponse;
        ...
        .catch((error) => {
            console.warn("Nixor Connect: failed to fetch Panapticon permissions", error);
